function obtenirLloguersManuals() {
      ...
      if (this.readyState == 4 && this.status == 200) {
        ...
        if (data == null) {
          document.querySelector("table").textContent =
            "No tens cap reserva feta o aprovada per junta";
        } else {
          for (let i = 0; i < data.length; i++) {
            ...
            if (data[i]["Recollida"] == null) {
              td[2].getElementsByTagName("button")[1].textContent =
                "Cancel·la lloguer";
            } else {
              td[2].getElementsByTagName("button")[1].textContent =
                "Retorna manual";
            ...
            if (data[i]["Recollida"] != null) {
              ...
              document
                .getElementById(data[i]["NumLloguer"])
    ...
    xhttp.open(
      "GET",
      "/api/lloguermanual.php?numsoci=" + sessionStorage["numsoci"],
      true
    );
    xhttp.send();
  }
