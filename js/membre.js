function actiuInactiu() {
  ...
  function desactivar() {
    ...
    if (
      confirm(
        "Estas segur?, si canvies d'idea envia un correu a juntacruab@gmail.com"
      )
    ) {
    ...
    xhttpmembres.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        for (let i = 0; i < membres.length; i++) {
          ...
          document
            .getElementById(membres[i]["numsoci"])
            .addEventListener("click", function () {
  ...
  function eliminarMembre(numsoci) {
    if (
      confirm("estas segur que vols eliminar el soci de numero " + numsoci + "?")
    ) {
