    ...
    xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        if (sessionStorage["juntari"] == "true") {
          document
            ...
            .addEventListener("click", function () {
              registreManual();
            }); /* esto no tendria que estar aqui, sino fuera del foreach*/
        ...
        llista.forEach(function (manual) {
          ...
          manualactual.getElementsByClassName("nom")[0].textContent = decode(
            manual.Nom
          );
          manualactual.querySelector("a").href =
            "manualindividual.php?nom=" + manual.Nom;
          manualactual.querySelector("img").src = manual.Imatge;
          manualactual.querySelector(".editorial").textContent =
            "Publicat per " + decode(manual.Editorial) + ".";
          manualactual.querySelector(".ambientacio").textContent =
            "Ambientació: " + decode(manual.Ambientacio) + ".";
          manualactual.querySelector(".coleccio").textContent =
            "Part de la col·leció " + decode(manual.Coleccio) + ".";
          manualactual.querySelector(".adquirit").textContent =
            "A la taquilla des del " + manual.Adquirit + ".";
          if (sessionStorage["juntari"] == "true") {
            manualactual.getElementsByClassName("eliminarboto")[0].textContent =
              "Eliminar manual";
            manualactual
              .getElementsByClassName("eliminarboto")[0]
              .classList.remove("hidden");
            manualactual
              .getElementsByClassName("eliminarboto")[0]
              .addEventListener("click", function () {
                eliminarManual(manual.Nom);
              });
          }
        });
      }
