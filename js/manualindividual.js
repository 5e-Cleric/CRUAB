function eliminarCopiaManual(copia) {
    if (confirm("Estas segur?")) {
      ...
      xhttpeliminarcopia.open(
        "DELETE",
        "/api/manualsindividuals.php?Nom=" + nom + "&NumCopia=" + copia,
        true
      );
  ...
  function reservarManual(manual, copia) {
    ...
    var data = {
      usuari: sessionStorage.getItem("numsoci"),
      manual: manual,
      copia: copia,
    };
    console.table(data);
      ...
      if (this.readyState == 4 && this.status == 200) {
        ...
        if (sessionStorage["numsoci"] != null) {
          ...
          llista.forEach(function (manual) {
            ...
            var copiaactual = llistacopies.querySelector(
              ".element-copia:last-child"
            );
            document.getElementById("imageHolder").querySelector("img").src =
              manual.Imatge;
            copiaactual.id = "copia" + manual.Numcopia;
            copiaactual.querySelector(".num-copia").textContent =
              "Copia " + manual.Numcopia;
            ...
            botoreserva.addEventListener("click", function () {
              reservarManual(nommanual, manual.Numcopia);
            });
            ...
            if (sessionStorage["juntari"] == "true") {
              ...
              botoelimina.addEventListener("click", function () {
                eliminarCopiaManual(manual.Numcopia);
              });
              if (manual.Retorn == null && manual.NumLloguer != null) {
          ...
          /*si no es membre, que inicii sessió*/
          window.location.href = "login.php";
        }
      }
    };
    xhttpindividual.open(
      "GET",
      "/api/manualsindividuals.php?nom=" + nommanual,
      true
    );
